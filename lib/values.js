...
ValueMap.set('bool', {
   ...
   read: (node, type) => readPrimitive(node, value => gaia.types.bool.value(value))
});

ValueMap.set('decimal', {
   write: (node, value, options) => writePrimitive(node, value, options),
   read: (node, type) => readPrimitive(node, value => gaia.types.decimal.value(value))
});

ValueMap.set('int', {
   write: (node, value, options) => writePrimitive(node, value, options),
   read: (node, type) => readPrimitive(node, value => gaia.types.int.value(value))
});

ValueMap.set('string', {
   write: (node, value, options) => writePrimitive(node, value, options),
   read: (node, type) => readPrimitive(node, value => gaia.types.string.value(value))
});

ValueMap.set('uint', {
   write: (node, value, options) => writePrimitive(node, value, options),
   read: (node, type) => readPrimitive(node, value => gaia.types.uint.value(value))
});
...
ValueMap.set('collection', {
   ...
   read: (node, type) => {
      ...
      if(type) {
         tpe = type;
         return gaia.types.collection.value(tpe.itemType);
      } else {
         tpe = types.read(node.ItemType[0]);
         return gaia.types.collection.value(tpe);
      }
   }
});
