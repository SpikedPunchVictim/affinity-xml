...
ValueMap.set('bool', {
    ...
    read: (node) => readSimple(node, value => gaia.types.bool.create(value))
});

ValueMap.set('decimal', {
    write: (node, value) => writeSimple(node, value),
    read: (node) => readSimple(node, value => gaia.types.decimal.create(value))
});

ValueMap.set('int', {
    write: (node, value) => writeSimple(node, value),
    read: (node) => readSimple(node, value => gaia.types.int.create(value))
});

ValueMap.set('string', {
    write: (node, value) => writeSimple(node, value),
    read: (node) => readSimple(node, value => gaia.types.string.create(value))
});

ValueMap.set('uint', {
    write: (node, value) => writeSimple(node, value),
    read: (node) => readSimple(node, value => gaia.types.uint.create(value))
});
...
ValueMap.set('collection', {
    ...
    read: node => {
        ...
        let collection = gaia.types.collection.create(type.itemType);
    }
});
...
function _write(node, value, opts) {
    ...
    if(!writer) {
        throw new Error('Unsupported Type encountered (writer): %s', value.type.name);
    }
...
function _read(node) {
   ...
   if(!reader) {
        throw new Error('Unsupported Type encountered (reader): %s', name);
    }
