...
var qpath = require('affinity').utils.qpath;
...
function write(projectDirectory, project) {
   return when.try(() => {
      ...
      return root.end(utils.formatXml());
   })
   .then(xmlString => utils.writeFile(path.join(projectDirectory, File.toc), xmlString, 'utf8'));
}
   ...
   return utils.readFile(tocFilePath, 'utf8')
      ...
      .then(toc => {
         ...
         for (let model of models) {
            let parent = qpath.parent(model.$.QualifiedName);
      ...
      .catch(err => {
         utils.printError('Failed reading toc: ', err);
      });
}
